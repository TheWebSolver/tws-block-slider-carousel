...
const createBullets = (container, bulletId) => {
  ...
  $element.setAttribute(
    'class',
    'tws-sliderCarousel__bullets swiper-pagination'
  );
...
const createArrows = (container, prev, next) => {
  ...
  $prevBtn.setAttribute(
    'class',
    'tws-sliderCarousel__arrow arrow--prev swiper-navigation'
  );
  $nextBtn.setAttribute('id', next);
  $nextBtn.setAttribute(
    'class',
    'tws-sliderCarousel__arrow arrow--next swiper-navigation'
  );
...
const hasContent = (list, values = [], isDOM = false, error = '') => {
  let $isValid = list.every((value, index, listAsValues) => {
    let $values =
      Array.isArray(values) && values.length === 0 ? listAsValues : values;

    // Verify that value is non-empty and is included in list.
    return isDOM
      ? $values.contains(value)
      : value.length > 0 && $values.includes(value);
  });
...
const addBreakpointClasses = (container, breakpoints) => {
  Object.entries(breakpoints).forEach((breakpoint) => {
    ...
    container.classList.add(
      Responsive.enabled ? `isEnabled__${Pixel}` : `isDisabled__${Pixel}`
    );
  });
};
...
const prepareData = (block, container, containerClass) => {
  ...
  if ($wrapperEl === null) {
    throw new Error(
      `Inside slider container with unique class "${containerClass}", wrapper element "${$wrapper}" not found.`
    );
  }
  ...
  try {
    ...
    if (
      isValidString($wrapperClass) &&
      !hasClass($wrapperEl.classList, $wrapperClass)
    ) {
      throw new Error(
        `Inside slider container with unique class "${containerClass}", slider wrapper element "<${$wrapperTag}>" for which "${$wrapperClass}" class(es) is set for removal doesn't exist.`
      );
    }
    ...
    if (
      hasClass($wrapperEl.classList, $wrapperClass) &&
      Array.isArray($wrapperClassList)
    ) {
  ...
  if ($slideCount < 2) {
    throw new Error(
      `Inside slider container with unique class "${containerClass}", there must be atleast 2 slide elements inside the wrapper element to initialize the slider.`
    );
  }
    ...
    sliderOptions.pagination = {
      ...
      ...(hasContent(
        ...
        renderBullet: (index, className) => {
          ...
          if (
            $bulletRender.length > 0 &&
            $bulletRender.length !== $slideCount
          ) {
            console.error($invalidBulletMsg);
          }

          let $bulletContent =
            $bulletRender.length === $slideCount ? $bulletRender[index] : '';
          return (
            '<span class="' + className + '">' + $bulletContent + '</span>'
          );
        }
      })
    };
  ...
  for (let slide = 0; slide < $wrapperChildNodes.length; slide++) {
    ...
    try {
      ...
      if ($getSlide[0] !== $nodeTag) {
        throw new Error(
          `Inside slider container with unique class "${containerClass}", the slide elements do not match. Element set on slider option is "<${$getSlide[0]}>" but "<${$nodeTag}>" found.`
        );
      }
      ...
      if ($slideNode.innerHTML === '') {
        console.info(
          `Inside slider container with unique class "${containerClass}", slide number "${$slideNumber}" has no content. Is that on purpose?`
        );
      }

      // Case where slide class to remove doesn't exist.
      if (
        isValidString($slideClass) &&
        !hasClass($slideNode.classList, $slideClass)
      ) {
        throw new Error(
          `Inside slider container with unique class "${containerClass}", slide element "<${$nodeTag}>" for which "${$slideClass}" class(es) is set for removal doesn't exist.`
        );
      }
...
if (Array.isArray(SliderElements)) {
  ...
  SliderElements.forEach((container, index) => {
    ...
    for (let block = 0; block < $blockElement.length; block++) {
      ...
      if (
        $blockElement[block].classList.contains('tws-block__sliderCarousel')
      ) {
        ...
        if ($blockElement[block].children.length !== 1) {
          console.error(
            `The block element with classname "${$blockClass}" does not have any inner HTML element to instantiate the slider. Use the block that creates an inner container like WordPress default "group" block.`
          );
        ...
        if (!$innerElement.classList.contains($blockChildClass)) {
          console.error(
            `The block element with classname "${container.parent}" does not have inner container element with classname as "${$blockChildClass}". Slider can't be initialized. These are passed inside function "tws_bfsc_get_elements()". If filter is used to modify it, make sure the returned elements are valid.`
          );
